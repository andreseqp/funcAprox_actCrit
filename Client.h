...
client::client(client_ty type, double means[], double sds[], double mins[], double probs[], double &rew)/*,double mMainRGB[], double sdMainRGB[], double &pSecCol,
...
void client::rebirth(client_ty type, double means[], double sds[], double mins[], double probs[], double &rew)
{
	mytype = type;
	if (mytype == absence)
	{
...
void client::printClientData(ofstream &learnSeries)														// prints data from the client into the learning trial
{
	learnSeries << mytype << '\t';
	for (size_t i = 0; i < 8; i++)
	{
